...
def render_game():
    ...
    return render_template("game.html", correct_text = round_text["correct_text"], eliminated_text = ''.join(round_text[1]), answer_text = ''.join(round_text[2]), host_text = ''.join(round_text[3]), question_text = ''.join((round_text[4])), incorrect_guesses_text= ''.join(round_text[5]),   col_size = col_size, col_sm_size= col_sm_size,  game_data =game_data)
