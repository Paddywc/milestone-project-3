...
def create_gameplay_lists(username_list, lives, score):
    ...
    and a boolean initially set as True(used to check if their last
...
def create_scores_tuple_list():
    ...
    with open("data/all_scores.txt", "r") as all_scores:
        for i in all_scores.readlines():
            tuple_entry = i.split(",")
            scores_tuple_list.append(
                (str(tuple_entry[0]), int(tuple_entry[1])))
...
def sort_scores(scores_tuple_list):
    ...
    sorted_list = sorted(scores_tuple_list, reverse=True,
